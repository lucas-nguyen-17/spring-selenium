...
import static com.giangtester.localhost.framework.locators.TagName.OPTION;
...
public interface FormElements extends ExplicitWait {
    ...
    default String getRadio(Supplier<By> by) {
        Optional<Element> radio =
                findElements(by)
                        .filter(e -> Boolean.parseBoolean(e.getAttribute("checked")))
                        .findFirst();

        return radio.map(DelegatingWebElement::getValue).orElse(null);
